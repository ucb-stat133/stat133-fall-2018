...
The data is in the file `nba2018-salary-points.RData`, located in the `data/` 
...
files in your working directory. If you run into problems, ask the GSI or any
...
__Your turn again.__ Create a vector `four` by selecting the first four elements in `player`:
...
vector `warriors_points` with their `scored` points. 
- Use `names()` to assign `warriors_player` as the names of `warriors_salary`.
