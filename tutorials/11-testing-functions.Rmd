...
knitr::opts_chunk$set(echo = TRUE, error = TRUE, fig.path = '11-images/')
library(testthat)
```
...
the functions provided by `"testthat"`. 
...
The organization the function files and the test files is (usually) up to you. The main exception is when you are creating an R package; in this case the functions are located in a specific file, while the tests are located in a separate specific location.
