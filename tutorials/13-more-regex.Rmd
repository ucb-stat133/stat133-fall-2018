...
contain the pattern `"jpg"`. We can use `str_count()` to count the number of matches in a string:
...
We can also use `str_detect()` to find what elements contain the pattern:
...
Likewise, we can take the previous about and pass it to `which()` to know the number of the element containing the pattern:
...
`.` corresponds to the _literal_ dot character. Recall that to match the dot, we need to escape it with `"\\."`:
...
Now let's try to count all types of image files (JPG, PNG, GIF, ICO) on the entire vector of `logs`
